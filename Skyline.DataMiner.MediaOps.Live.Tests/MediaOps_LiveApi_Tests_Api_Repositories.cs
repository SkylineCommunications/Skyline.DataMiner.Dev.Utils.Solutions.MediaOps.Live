namespace Skyline.DataMiner.MediaOps.Live.Tests
{
	...
	[TestClass]
	public sealed class MediaOps_LiveApi_Tests_Api_Repositories
	{
		[TestMethod]
		...
		public void MediaOps_LiveApi_Tests_Read_Ids()
		{
			...
			Assert.HasCount(2, endpoints_read);
		...
		public void MediaOps_LiveApi_Tests_Read_Query()
		{
			...
			Assert.HasCount(20, endpoints_filter);

			var endpoints_query = api.Endpoints.Read(endpointQuery).ToList();
			Assert.HasCount(5, endpoints_query);
		}
		...
		public void MediaOps_LiveApi_Tests_JoinMultiple()
		{
			...
			foreach (var item in result)
			{
				...
				{
					Assert.EndsWith(vsg.Name, endpoint.Name, $"Endpoint '{endpoint.Name}' should end with VSG name '{vsg.Name}'.");
				}
			}
		}
	}
