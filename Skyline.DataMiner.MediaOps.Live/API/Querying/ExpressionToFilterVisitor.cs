namespace Skyline.DataMiner.MediaOps.Live.API.Querying
{
		...
		where T : ApiObject<T>
	{
		...
		private ExpressionToFilterVisitor(Repository<T> repository)
		{
			_repository = repository ?? throw new ArgumentNullException(nameof(repository));
		}

		public static FilterElement<DomInstance> GetFilter(Expression expression, Repository<T> repository)
		{
			var visitor = new ExpressionToFilterVisitor<T>(repository);
		...
		protected override Expression VisitMethodCall(MethodCallExpression node)
		{
				...
				ExpressionTools.TryGetValue(arguments[0], out var value))
			{
				...
				var filter = _repository.CreateFilter(fieldName, Comparer.Contains, value);
				_filters.Add(filter);
			}
