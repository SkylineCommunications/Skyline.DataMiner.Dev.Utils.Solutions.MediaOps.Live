namespace Skyline.DataMiner.Solutions.MediaOps.Live.Automation.Orchestration.Script
{
	...
	/// <summary>
	/// Shared logic of <see cref="OrchestrationScript"/> and <see cref="DynamicOrchestrationScript"/>. Derive from one of those instead.
	/// </summary>
	public abstract class OrchestrationScriptBase
	{
		...
		internal OrchestrationScriptBase()
		{
		}

		public OrchestrationEventConfiguration EventConfiguration => _eventConfiguration?.Value;

		internal List<ParameterInfo> ParameterInfos => _parameterInfos;
		...
		internal abstract IEnumerable<IOrchestrationParameters> GetProfileParameters();
		...
		private OrchestrationScriptInfo GetScriptInfo(OrchestrationInputValues providedValues)
		{
			...
			foreach (IOrchestrationParameters orchestrationParameters in GetProfileParameters())
			{
